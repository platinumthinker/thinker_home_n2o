...
main() ->
    case wf:user() of
         ...
         _ -> #dtl{file = "index", app=thinker_home,
                   bindings=[{body,body()},{list,content()}]} end.
...
list() ->
    ...
    #ul{body=[ #li{body=#link{body=filename:basename(File),
     ...
     || File<-filelib:wildcard(code:priv_dir(thinker_home)++"/snippets/"++Room++"/*") ]}.
...
event({client,{User,Message}}) ->
    ...
    DTL = #dtl{file="message",app=thinker_home,
